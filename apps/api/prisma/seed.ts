...
export async function runSeed(prisma: PrismaClient): Promise<void> {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const gazePoints = [];
    for (let k = 0; k < 200; k++) {
      const cx = k % 2 === 0 ? 0.4 : 0.6;
      const cy = k % 3 === 0 ? 0.3 : 0.7;
      gazePoints.push({
        sessionId: session.id,
        tMs: k * 50,
        x: Math.max(0, Math.min(1, cx + (Math.random() - 0.5) * 0.3)),
        y: Math.max(0, Math.min(1, cy + (Math.random() - 0.5) * 0.3)),
      });
    }
    await prisma.gazeData.createMany({ data: gazePoints });
  }
